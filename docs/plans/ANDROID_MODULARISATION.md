...
Status: **Phases 0 and 2 landed; Phases 1, 3 and 4 blocked pending a quiet tree.**
...
every other. `feature/nutrition` already reaches into `feature/dashboard` for
five symbols, and three of those (`MacroRow`, `HydrationCard`, `CardSkeleton`)
are declared `internal` — a modifier that will only start meaning something
once the modules are real.

## Measured dependency graph

Derived by extracting every `import fit.aquazero.app.{core,feature}.*` edge.

**Status as re-measured 2026-08-29.** Five of the six violations below are
resolved in code; only the `nutrition -> dashboard` move remains. Re-measure before trusting this table again — it drifted
badly once already, and the edge counts move whenever a feature grows.
...
| 3 | `feature:nutrition` depends on `feature:dashboard` | **23** imports (not 12) of `NutritionFormat`, `CardSkeleton`, `HydrationCard`, `MacroRow`, `rememberToastSink` | Move all five to `:core:ui`. | OPEN — grew since first measured |
...
Remaining feature-to-feature edges, all one-way into `dashboard`, so the
graph is now an acyclic fan-in:

```
challenges -> dashboard   (1)
coach      -> dashboard   (2)
coach      -> gamification(3)
nutrition  -> dashboard  (23)
...
Nothing imports it yet, but training and challenges both will — placing it in
`:core:ui` prevents the next feature-to-feature edge by construction.

## The largest piece: 643 string resources

Library modules cannot see `fit.aquazero.app.R`. 51 files import it. Measured
partition of the string table:
...
**Phase 1 — break the violations while still single-module.** Three separate
commits: (1a) extract the `:core:model` contents in place, (1b) invert
auth/network via the SPI interfaces, (1c) move the five shared symbols to a
`core/ui` package. Verify `./gradlew assembleDebug test` after each. Nothing
about the Gradle structure changes yet, so each step is independently
