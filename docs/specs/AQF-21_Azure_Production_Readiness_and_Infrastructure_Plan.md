# AquaZeroFit — Production Readiness Review & Azure Infrastructure Plan
...
Sections 1–3 are assessment. Sections 4–9 are the build. Section 10 onward is cost, risk and sequencing.
...
Estimated **3–5 focused days** for steps 1–4 and **~2 days** for step 5. The existing test suite is what makes this safe.
...
| Safety guardrail `pre()` is called on the chat lane only; vision, recommendations, plans and memory extraction bypass it. The "safety classifier" is pure regex; P-09 is loaded only to produce a version string | Safety coverage is far narrower than it appears | 1–2 days |
...
**Phase 1 — Make it deployable (1–2 weeks).** Cosmos migration per §3.3. Blob Storage for meal photos. Redis for rate limits. Upgrade multer. Add CI with the eval gate. Deploy to a staging environment and keep it at one replica until Cosmos lands.
